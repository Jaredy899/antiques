...
export default function VendorsPage() {
  ...
  useEffect(() => {
    async function loadImages() {
      try {
        const images = await getImagesFromFolder('vendors');
        setImages(images);
      } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-logo-800">Vendor Gallery</h1>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {images.map((imageUrl, index) => (
          ...
          >
            <Image
              src={imageUrl}
              alt={`Vendor booth image ${index + 1}`}
      ...
      {images.length === 0 && (
        <p className="text-center text-logo-600">
          No vendor booth images available.
        </p>
      )}
